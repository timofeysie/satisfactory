...
Currently if this happesn now it wont break the post.  But then we would have to manually upload the image we want and create the s3 bucket property on the json.  I suppose in this case we want to add a blank s3 property and let the user fill it in.
